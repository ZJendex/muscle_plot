...
INPUT_DIR = "benchmark_isometric\\environment"  # 当前目录，可以修改为其他路径，例如: "./server" 或 "D:/path/to/json/files"
...
if __name__ == "__main__":
    ...
    # ==================== Create Environment Benchmark Bar Plot (RMSE) ====================
    print("\nCreating Environment Benchmark bar plot for RMSE...")
    
    # Extract environment/scenario information from filenames and group by environment
    environment_groups = {}
    for result in all_results:
        filename = result['subject']
        
        # Try to extract scenario from filename (e.g., "scenario1", "scenario2", "scenario3")
        environment = None
        if 'scenario1' in filename.lower() or 'scenario_1' in filename.lower():
            environment = 'Scenario 1'
        elif 'scenario2' in filename.lower() or 'scenario_2' in filename.lower():
            environment = 'Scenario 2'
        elif 'scenario3' in filename.lower() or 'scenario_3' in filename.lower():
            environment = 'Scenario 3'
        elif 'scenario4' in filename.lower() or 'scenario_4' in filename.lower():
            environment = 'Scenario 4'
        elif 'scenario5' in filename.lower() or 'scenario_5' in filename.lower():
            environment = 'Scenario 5'
        
        if environment:
            if environment not in environment_groups:
                environment_groups[environment] = []
            environment_groups[environment].append(result)
            print(f"  {filename} -> {environment}")
    
    # Initialize variables for final print statements
    environment_plot_exists = False
    environment_rmse_mvic_filename = None
    environment_rmse_kg_filename = None
    bar_data_filename = None
    
    if environment_groups:
        # Define environment order
        environment_order = ['Scenario 1', 'Scenario 2', 'Scenario 3', 'Scenario 4', 'Scenario 5']
        environments = [e for e in environment_order if e in environment_groups]
        
        # Calculate mean RMSE for each environment (MVIC %)
        rmse_mvic_by_environment = []
        rmse_mvic_std_by_environment = []
        
        for env in environments:
            results_at_env = environment_groups[env]
            rmse_values = [r['rmse_mvic_pct'] for r in results_at_env]
            ...
            if len(rmse_values) == 1:
                ...
                rmse_mvic_std_by_environment.append(rmse_std)
            else:
                # Multiple files: use std across files
                rmse_mvic_std_by_environment.append(np.std(rmse_values, ddof=1))
            
            rmse_mvic_by_environment.append(np.mean(rmse_values))
            print(f"  {env}: {len(results_at_env)} file(s), RMSE = {np.mean(rmse_values):.4f} ± {rmse_mvic_std_by_environment[-1]:.4f} %MVIC")
        
        # Save bar plot data to CSV
        import csv
        bar_data_filename = os.path.join(OUTPUT_DIR, f"environment_benchmark_data{config_suffix}.csv")
        with open(bar_data_filename, 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['Environment', 'RMSE_MVIC_pct', 'STD_MVIC_pct', 'Num_Files'])
            for i, env in enumerate(environments):
                writer.writerow([env, f"{rmse_mvic_by_environment[i]:.4f}", 
                               f"{rmse_mvic_std_by_environment[i]:.4f}", 
                               len(environment_groups[env])])
        ...
        # Professional color palette - multiple colors for scenarios
        colors = ['#4472C4', '#ED7D31', '#A5A5A5', '#FFC000', '#5B9BD5']  # Professional colors
        
        bars = ax.bar(x_pos, rmse_mvic_by_environment, yerr=rmse_mvic_std_by_environment,
                      capsize=5, alpha=0.8, color=colors[:len(environments)],
                      edgecolor='black', linewidth=1.0,
                      error_kw={'linewidth': 1.5, 'ecolor': 'black', 'capthick': 1.5})
        
        # Add value labels on top of bars - cleaner style
        for i, (rmse, std) in enumerate(zip(rmse_mvic_by_environment, rmse_mvic_std_by_environment)):
            ax.text(i, rmse + std + 0.3, f'{rmse:.1f}%',
                   ha='center', va='bottom', fontsize=11)
        
        # Formatting - clean and professional
        ax.set_xlabel('Environment', fontsize=13)
        ax.set_ylabel('RMSE (%MVIC)', fontsize=13)
        ax.set_title('GigaFlex Environment Benchmark', fontsize=14, pad=15)
        ...
        plt.tight_layout()
        environment_rmse_mvic_filename = os.path.join(OUTPUT_DIR, f"environment_benchmark_rmse_mvic{config_suffix}.png")
        plt.savefig(environment_rmse_mvic_filename, dpi=300, bbox_inches='tight', facecolor='white')
        print(f"Saved Environment Benchmark RMSE plot (MVIC %) to: {environment_rmse_mvic_filename}")
        plt.close()
        
        # Create RMSE bar plot for kg (if available)
        if results_with_mvic:
            # Calculate mean RMSE for each environment (kg)
            rmse_kg_by_environment = []
            rmse_kg_std_by_environment = []
            
            for env in environments:
                results_at_env = [r for r in environment_groups[env] if r['rmse'] is not None]
                if results_at_env:
                    rmse_values_kg = [r['rmse'] for r in results_at_env]
                    ...
                    if len(rmse_values_kg) == 1:
                        ...
                        rmse_std = np.std(errors, ddof=1) / np.sqrt(2)
                        rmse_kg_std_by_environment.append(rmse_std)
                    else:
                        # Multiple files: use std across files
                        rmse_kg_std_by_environment.append(np.std(rmse_values_kg, ddof=1))
                    
                    rmse_kg_by_environment.append(np.mean(rmse_values_kg))
                    print(f"  {env}: {len(results_at_env)} file(s), RMSE = {np.mean(rmse_values_kg):.4f} ± {rmse_kg_std_by_environment[-1]:.4f} kg")
                else:
                    rmse_kg_by_environment.append(0.0)
                    rmse_kg_std_by_environment.append(0.0)
            
            # Add kg data to the same CSV file
            with open(bar_data_filename, 'a', newline='', encoding='utf-8') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([])  # Empty row
                writer.writerow(['Environment', 'RMSE_kg', 'STD_kg', 'Num_Files'])
                for i, env in enumerate(environments):
                    writer.writerow([env, f"{rmse_kg_by_environment[i]:.4f}", 
                                   f"{rmse_kg_std_by_environment[i]:.4f}", 
                                   len([r for r in environment_groups[env] if r['rmse'] is not None])])
            ...
            bars = ax.bar(x_pos, rmse_kg_by_environment, yerr=rmse_kg_std_by_environment,
                          capsize=5, alpha=0.8, color=colors[:len(environments)],
                          edgecolor='black', linewidth=1.0,
                          error_kw={'linewidth': 1.5, 'ecolor': 'black', 'capthick': 1.5})
            
            # Add value labels on top of bars - cleaner style
            for i, (rmse, std) in enumerate(zip(rmse_kg_by_environment, rmse_kg_std_by_environment)):
            ...
            plt.tight_layout()
            environment_rmse_kg_filename = os.path.join(OUTPUT_DIR, f"environment_benchmark_rmse_kg{config_suffix}.png")
            plt.savefig(environment_rmse_kg_filename, dpi=300, bbox_inches='tight', facecolor='white')
            print(f"Saved Environment Benchmark RMSE plot (kg) to: {environment_rmse_kg_filename}")
            plt.close()
            
            environment_plot_exists = True
        else:
            environment_plot_exists = False
            environment_rmse_kg_filename = None
    else:
        print("Warning: Could not extract environment information from filenames!")
    ...
    if environment_groups and bar_data_filename:
        print(f"  - {environment_rmse_mvic_filename}")
    ...
    if environment_plot_exists:
        print(f"  - {environment_rmse_kg_filename}")
    print(f"{'='*100}")
