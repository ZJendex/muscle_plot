...
if __name__ == "__main__":
    ...
    for result in all_results:
        # Extract subject name (use short name for better display)
        subject_name = result['subject'].split('_')[1] if '_' in result['subject'] else result['subject']
        subjects.append(subject_name)
        
        # MAE is already calculated
        mae_values_mvic.append(result['mae_mvic_pct'])
        
        # Calculate standard deviation of errors for this subject
        errors = result['errors_mvic_pct']
        mae_std_values_mvic.append(np.std(errors))
    ...
    if results_with_mvic:
        ...
        for result in results_with_mvic:
            ...
            # MAE in kg
            mae_values_kg.append(result['mae'])
            
            # Calculate standard deviation of errors in kg
            errors = result['errors']
            mae_std_values_kg.append(np.std(errors))
